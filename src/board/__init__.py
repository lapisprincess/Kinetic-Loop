...
class Board(pg.Rect):
    ...
    def update(self, check_input = False):
        ...
        for entity in self.entities.sprites():
            if type(entity) == Player:
                self.player = entity
                if self.visibility: fov_los(self, entity, 6 * self.tile_width)
    ...
    def tile_at_pixel(self, pixel_coord):
        for thing in self.get_everything():
            if type(thing) != tile.Tile: continue
            if thing.pixel_collide(pixel_coord): return thing
        return None

    def entity_at_pixel(self, pixel_coord):
        for entity in self.entities.sprites():
            if entity.pixel_collide(pixel_coord): return entity
        return None
